...
export class MutedService {
  ...
  validate(otherUserId) {
    return new Promise(resolve => {
      this.apiProvider.getAuthentication(true).then(auth => {
        if (auth['error'] == 0) {
          let url = auth['data']['data_url'] + '/muted/validate/otherUserId/'+otherUserId;
          ...
          let groupKey = 'api_muted_validate_otheruserid_'+otherUserId;
  ...
  save(data) {
    return new Promise(resolve => {
      this.apiProvider.getAuthentication(true).then(auth => {
        if (auth['error'] == 0) {
          let url = auth['data']['data_url'] + '/muted/save';
  ...
  getList(otherUserId?, page = 1, limit = 10, search?) {
    return new Promise(resolve => {
      this.apiProvider.getAuthentication(true).then(auth => {
        if (auth['error'] == 0) {
          let url = auth['data']['data_url'] + '/muted/listing/page/' + page + '/limit/' + limit;
          if (otherUserId) {
          ...
          let groupKey = 'api_muted_listing_page_'+page+'_limit_'+limit;
          if (otherUserId) {
