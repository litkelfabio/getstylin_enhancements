...
import { AlertController, IonImg, IonRefresher, LoadingController, ModalController, NavController } from '@ionic/angular';
...
isLoadedReward = false;
  ...
  constructor(
    ...
    //   this.reward_id = data['reward_id'] ? data['reward_id'] : null;
    //   this.user_id = data['user_id'] ? data['user_id'] : null;
    //   console.log("rewards_id", this.reward_id );
  ...
  async getReward(params){
    ...
    console.log(params);
    try{
      this.rewards.getDetails(params).then( async res => {
        let e:any = res;
  ...
  back(){
    if(this.context == 'from-market'){
      this.navCtrl.navigateBack('/tabs/tabs/rewards');
    }else{
      this.navCtrl.navigateBack('/tabs/tabs/rewards/my-rewards');
    }
  }
  openUrl(){ 
    let target = "_blank";
